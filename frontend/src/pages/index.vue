...
const go = () => {
  axios.get("https://cen7l2mcx6.execute-api.us-east-1.amazonaws.com/stage/echo").then((res) => {
    ...
    if (res.data.length > 0) {
      videos.value = s3HLSObjects.value.map((obj) => { return { url: obj.Key, LastModified: obj.LastModified } })
...
<template>
  <div>
    ...
    <div>
      <button class="m-3 text-sm btn" @click="go">Get Videos</button>

      <button class="m-3 text-sm btn" @click="goToUpload">Upload</button>
    </div>
    <div class="m-6">
